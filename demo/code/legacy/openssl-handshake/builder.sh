...
if [ "$1" = "clean" ]; then
    ...
    if test target/$EXECUTEABLE; then
        cd target
        valgrind --tool=callgrind --dump-instr=yes --simulate-cache=yes --collect-jumps=yes --callgrind-out-file="callgrind-report" echo wikipedia.org | ./$EXECUTEABLE
