                    ...
                    if(extension === ".cpp"){
                        ...
                        try{
                            ...
                            if(compileError){
                                ...
                                let graphCommand = `${APPPATH} analyze --profile ${profilepath} --mode graph --format json --strategy ${STRATEGY} --loopbound ${LOOPBOUND} --withCalls --program ${PROJECTDIR}/compiled/${filename}.ll > ${PROJECTDIR}/graph.dot`;
                                if(params && params.functionname !== ""){
                                    console.log(params.functionname);

                                    graphCommand = `${APPPATH} analyze --profile ${profilepath} --mode graph --format json --strategy ${STRATEGY} --loopbound ${LOOPBOUND} --withCalls --forFunction ${params.functionname} --program ${PROJECTDIR}/compiled/${filename}.ll > ${PROJECTDIR}/graph.dot`;
                                }
