...
import { ConfigParser, ProfileOptions } from '../helper/configParser';
import { Logger, LogType } from '../helper/logger';
...
export default async function profile() {
    ...
    if(ConfigParser.validateConfig()){
        const profileOptions: ProfileOptions | undefined = ConfigParser.getProfileOptions();

        if(profileOptions !== undefined){
            if(!fs.existsSync(`${PROJECTDIR}`)){
                ConfigParser.createSpearDir();
            }
            ...
            Logger.log(LogType.ERROR, "Parsing of config failed. Could not get configuration for the profiling!");
        }
    }
}